...
contract BOOBSOFBITCOIN is ERC721, ERC721URIStorage, Ownable, ERC2981{
    ...
    mapping (address => bool) internal pointManagers;
    ...
    function addBonusAllocation(uint id, uint amount) public returns(bool){
        require(pointManagers[msg.sender] || msg.sender ==owner(),"not allowed");
        extraData[id].bonusAllocation += amount; 
        return true;
    }

    function subBonusAllocation(uint id, uint amount) public returns(bool){
        require(pointManagers[msg.sender] || msg.sender ==owner(),"not allowed");
        extraData[id].bonusAllocation -= amount; 
        return true;
    }
